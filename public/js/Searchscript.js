    ...
    for (i = 0; i < document.styleSheets.length; i++) {
        ...
        for (j = 0; j < r.length; j++) {
            ...
            for (k = 0; k < s.rules.length; k++) {
                var css = s.rules[k].cssText || s.rules[k].mainpage100style.cssText;
                if (!css) {
                    continue;
                }
...
jQuery(function ($) {
    ...
    if (!browser.ie || browser.version > 8)
        return;
    var path = "";
    var scripts = $("script[src*='mainpage100script.js']");
    if (scripts.length > 0) {
        var src = scripts.last().attr('src');
        path = src.substr(0, src.indexOf("mainpage100script.js"));
    }
